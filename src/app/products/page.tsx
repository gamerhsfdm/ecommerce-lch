...
import { ListFilter, LayoutGrid, SlidersHorizontal, ChevronDown, ShoppingCart } from "lucide-react";
...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
    ...
    return (
        <Card className="flex flex-col overflow-hidden transition-shadow duration-300 hover:shadow-xl group">
            <Link href={`/product/${product.id}`} className="relative block overflow-hidden h-48">
                <img
            ...
            <CardHeader className="p-4 pb-0 flex-grow">
                <h3 className="text-lg font-semibold line-clamp-2">
                    <Link href={`/product/${product.id}`} className="hover:text-teal-700 transition-colors">
                        {product.name}
                    </Link>
                </h3>
            </CardHeader>
            ...
            <CardContent className="p-4 pt-2">
                <p className="text-2xl font-bold text-teal-700 mb-2">{formattedPrice}</p>
                
                {/* Área para Rating (simplificado) */}
                <div className="flex items-center text-yellow-500 text-sm">
                    {Array.from({ length: 5 }).map((_, i) => (
                        <svg key={i} className={cn("w-4 h-4", i < product.rating ? "fill-yellow-500" : "fill-gray-300")} viewBox="0 0 24 24"><path d="M12 17.27L18.18 21L16.54 13.97L22 9.24L14.81 8.62L12 2L9.19 8.62L2 9.24L7.46 13.97L5.82 21L12 17.27Z"/></svg>
                    ))}
                    <span className="ml-2 text-gray-500 text-xs">({product.rating}.0)</span>
                </div>
            </CardContent>
...
const FilterGroup: React.FC<FilterGroupProps> = ({ title, children }) => {
    ...
    return (
        <div className="py-4 border-b">
            ...
            >
                {title}
                <ChevronDown size={20} className={cn("transition-transform", isOpen ? "rotate-180" : "rotate-0")} />
            </button>
            <div className={cn("mt-3 overflow-hidden transition-all duration-300", isOpen ? "max-h-96" : "max-h-0")}>
                {children}
            </div>
        </div>
    );
    ...
    return (
        <div className="bg-gray-50 min-h-screen">
            <div className="container mx-auto px-4 py-10 md:py-16">
                ...
                {/* Título e Controles */}
                <div className="mb-8">
                    <h1 className="text-4xl font-extrabold text-gray-900">Resultados da Busca</h1>
                    <p className="text-gray-500 mt-1">Exibindo {paginatedProducts.length} de {mockProducts.length} produtos.</p>
                </div>
                            ...
                            <div className="space-y-3 text-sm">
                                {categories.map(category => (
                                    <div key={category} className="flex items-center space-x-2">
                                        ...
                                        />
                                        <label htmlFor={`cat-${category}`} className="text-gray-700 hover:text-gray-900 cursor-pointer">
                                            {category}
                                        </label>
                                    </div>
                                ))}
                            ...
                            <div className="space-y-3 text-sm">
                                {brands.map(brand => (
                                    <div key={brand} className="flex items-center space-x-2">
                                        <Checkbox id={`brand-${brand}`} className="data-[state=checked]:bg-teal-700" />
                                        <label htmlFor={`brand-${brand}`} className="text-gray-700 hover:text-gray-900 cursor-pointer">
                                            {brand}
                                        </label>
                                    </div>
                                ))}
